...
def brightness_tuner(image_path):
    # Load the image
    frame = cv2.imread(image_path)
    if frame is None:
        print("❌ Error: Could not load image.")
    ...
    cv2.namedWindow("Brightness Tuner")

    def nothing(x):
        pass

    # Trackbars for tuning
    cv2.createTrackbar("Brightness Threshold", "Brightness Tuner", 180, 255, nothing)
    cv2.createTrackbar("Blur Kernel", "Brightness Tuner", 5, 30, nothing)  # must be odd
    cv2.createTrackbar("Min Area", "Brightness Tuner", 5, 200, nothing)

    print("💡 Adjust sliders to tune bright spot detection. Press 'q' to quit.")

    while True:
        # Get current trackbar positions
        thresh_val = cv2.getTrackbarPos("Brightness Threshold", "Brightness Tuner")
        blur_k = cv2.getTrackbarPos("Blur Kernel", "Brightness Tuner")
        blur_k = max(1, blur_k // 2 * 2 + 1)  # ensure odd kernel size
        min_area = cv2.getTrackbarPos("Min Area", "Brightness Tuner")

        # Bright spot mask
        _, bright_mask = cv2.threshold(v_channel, thresh_val, 255, cv2.THRESH_BINARY)
        bright_mask = cv2.GaussianBlur(bright_mask, (blur_k, blur_k), 0)
        ...
        for c in contours:
            ...
            if area > min_area:
                ...
                cv2.putText(vis, f"{int(area)}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)

        # Display results
        cv2.imshow("Original", frame)
        cv2.imshow("Brightness Mask", bright_mask)
        cv2.imshow("Detected Bright Spots", vis)

        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            print(f"✅ Final Brightness Threshold: {thresh_val}, Blur Kernel: {blur_k}, Min Area: {min_area}")
            print(f"💡 Detected {bright_count} bright spots.")
            break
...
if __name__ == "__main__":
    test_image_path = "frame_0069_raw.jpg"  # Your test frame
